...
Utilizamos Docker Compose para configurar la infrastructura necesaria para la demo, que necesita los siquites componentes

- Mysql (fuente de datos)
- Postgres (destino de datos, replica)
- Kafka
  - Zookeeper.
  - Kafka Broker.
  - Kafka Connect con [Debezium](https://debezium.io/), para Postgres utilizamos [el conector de Kafka Connect para JDBC](https://github.com/confluentinc/kafka-connect-jdbc).
  - kafdrop For UI to Kafka topics.

## Diagrama de la arquitectura empleada
...
./init.sh
```
### Creamos un registro con valores arbitrarios que luego insertaremos en MySQL
```shell
docker exec -it demo_cdc_crist_mysql_1 bash -c 'bash /app/create_1_insert.sh'
```
### Comprobamos  MySQL

Comprobamos que la tabla `cr_th_activo` ha sido creada en MySQL e insertamos 1 registro
